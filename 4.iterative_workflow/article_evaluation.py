...
class ArticleEvalState(BaseModel):
    article: str
    language_usage_score: int = 0
    clarity_of_thoughts_score: int = 0
...
def regenerate_article(state: ArticleEvalState):
    prompt = PromptTemplate(
        template="Generate an improved version of given article \n {article} in 200 words",
        input_variables=['article']
    )

    chain = prompt | generator_llm | parser
    result = chain.invoke({"article": state.article})
    return {'article': result, 'iteration': state.iteration + 1}
...
def evaluate_final_score(state: ArticleEvalState):
    if state.final_score > 7 or state.iteration > state.max_iterations:
        return "approved"
    else:
...
graph.add_node("regenerate_article", regenerate_article)
...
graph.add_conditional_edges("get_final_score", evaluate_final_score, {"approved": END, "needs_improvement": "regenerate_article"})
graph.add_edge("regenerate_article", "get_language_usage_score")
graph.add_edge("regenerate_article", "get_clarity_of_thoughts_score")
graph.add_edge("regenerate_article", "get_content_depth_score")
...
initial_state = ArticleEvalState(
    article=article,
    iteration=1,
    max_iterations=3
)
